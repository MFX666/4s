...
public class CarSaleInfoController extends BaseController {
    ...
    * */
    ...
    public ResponseBean updateCarSaleInfo(@RequestBody CarSaleInfo carSaleInfo){
        if(carSaleInfo==null) return ResponseBean.error("参数为空");
        if(carSaleInfoService.getById(carSaleInfo).getIsSettlement().equals("已结算")) return ResponseBean.success("已结算订单不能修改");
        if(carSaleInfoService.updateById(carSaleInfo)) return ResponseBean.success("修改成功");
    ...
    public ResponseBean deleteCarSaleInfo(@PathVariable Integer id){
        ...
        if(target.getIsSettlement().equals("已结算")) return ResponseBean.error("订单已结算无法删除");
    ...
    public ResponseBean payMoney(@PathVariable Integer id){
        if(id==null) return ResponseBean.error("参数为空");
        CarSaleInfo target = carSaleInfoService.getById(id);
        if(target.getIsSettlement().equals("已结算")) return ResponseBean.error("已结算请勿重复结算");
        CarStorage carStorage = carStorageService.getById(target.getCarStorageId());
        carStorage.setIsEnable(0);
        if(carStorageService.save(carStorage)) return ResponseBean.success("结算成功");
